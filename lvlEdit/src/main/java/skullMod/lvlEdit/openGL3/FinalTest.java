...
import skullMod.lvlEdit.openGL.SimpleShaderProgram;
...
// Set the viewport to be the entire window
        ...
        Mini_GLUT.recalculateProjectionMatrix(w,h,53.13f, 1.0f, 30.0f, temp);
        projMatrix = temp.get();
    }
...
// must be called after glUseProgram
    ...
    void setupShaders(GL3 gl) {
        SimpleShaderProgram shader = new SimpleShaderProgram("test", Mini_GLUT.loadFileAsString("C:\\shaders\\color.vert"),Mini_GLUT.loadFileAsString("C:\\shaders\\color.frag"), gl);

        this.shaderProgram = shader;

        vertexLoc = gl.glGetAttribLocation(shaderProgram.shaderProgramID, "position");
        colorLoc = gl.glGetAttribLocation(shaderProgram.shaderProgramID, "color");

        projMatrixLoc = gl.glGetUniformLocation(shaderProgram.shaderProgramID, "projMatrix");
